...
void switchToResistance(struct State* state) {
	...
	PB_LCD_GoToXY(0,0);
	PB_LCD_WriteString("Resistance", 11);
...
void updateResistance(struct State* state) {
	...
	//float resistance = scale(averageReading(), 0, ADC_MAX, RESISTANCE_MIN, RESISTANCE_MAX);
	float resistance = scale(ADC_Read(), 0, ADC_MAX, RESISTANCE_MIN, RESISTANCE_MAX);
	
	// Display the resistance
	PB_LCD_GoToXY(0, 1);
	char text [7];
	snprintf(text, 7, "%.3fR", resistance);
